...
class PlayingField:
    def __init__(self, field, row_g, game_run, sum_pl_1, sum_pl_2, cross_count, turn, player):
        self.field = field
        self.row_g = row_g
    ...
    def click(self, row, col):
        if self.game_run and self.field[row][col]['text'] != ' ' and self.field[row][col]['text'] != 'X' and \
                self.field[row][col]['background'] == 'red' and self.cross_count != self.row_g * self.row_g:

            if self.player == 0:
                self.sum_pl_1 += int(self.field[row][col]['text'])
                ttk.Label(frame2,  text="Игрок 2 - " + str(self.sum_pl_2), background='red').grid(column=0, row=8 * 10)
                ttk.Label(frame1, text="Игрок 1 - " + str(self.sum_pl_1),
                          background='lavender').grid(column=0, row=4 * 10)
                print(self.cross_count, " sum_pl_1", self.sum_pl_1)
                self.player = 1
            else:
                self.sum_pl_2 += int(self.field[row][col]['text'])
                ttk.Label(frame1, text="Игрок 1 - " + str(self.sum_pl_1), background='red').grid(column=0, row=4 * 10)
                ttk.Label(frame2, text="Игрок 2 - " + str(self.sum_pl_2),
                          background='lavender').grid(column=0, row=8 * 10)
...
new_button.grid(row=size+1, column=0, columnspan=size, sticky='nsew')
